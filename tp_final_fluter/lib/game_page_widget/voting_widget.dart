...
class _VotingGalleryState extends ConsumerState<VotingGallery> {
  ...
  Widget build(BuildContext context) {
    ...
    // Trouve le vote du joueur courant (s'il existe)
      ...
      data: (photos) {
        return GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final isOwnPhoto = photo.playerId == currentUserId;
            final isSelected = myVote?.votedForPlayerId == photo.playerId;
            final voteCount = votes.where((v) => v.votedForPlayerId == photo.playerId).length;

            final canTap = !isOwnPhoto && !hasVoted && !_isSubmitting && currentUserId != null;

            return GestureDetector(
              onTap: canTap ? () => _vote(photo.playerId, currentUserId) : null,
              child: Stack(
                children: [
                  Container(
